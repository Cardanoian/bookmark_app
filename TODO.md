...
- **테스트 494 runs / 0 failures**, rubocop 무경고, brakeman 0. (전체 점검 Phase 0~3 하드닝 + Gemini API 실동작화 반영)
...
> 남은 작업을 **리스크·의존성·비용** 기준으로 정렬한 권장 순서. 각 항목은 아래 카테고리 섹션에 상세 정의가 있다.

- [x] ~~**하드닝 브랜치 → main 반영**~~ — Phase 0~3 5커밋 로컬 main 병합 완료(미push).
- [x] ~~**외부 API 실연동 검증**~~ — 세 API 실호출 검증 완료, Gemini 결함 2건 수정(→ 🔵 외부 API 섹션).
1. **🟡 몬스터 phase-2 시드** — 나머지 12라인×3폼=36폼 추가. **왜 먼저:** 분모가 24라인이라 현재 `dex_complete`가 영구 미달 → 게이미피케이션 완성 보상 루프가 닫히지 않는 실질 결함. 데이터 작업이라 범위 명확.
2. **🔵 OCR 실이미지 검증** — 유일하게 남은 미검증 API 경로. 실사진 업로드로 `response["text"]` 매핑 첫 확인. 저비용.
3. **🔴 배포 준비** — 실 도메인·메일러 호스트·프로덕션 시크릿·Active Storage·SQLite 볼륨/백업. 드로플릿·레지스트리 자격 확보되면 착수(마일스톤).
4. **🔵 모니터링·에러 트래킹** — 프로덕션 로깅/에러 리포팅 도입. 보류항목 **3.2 착수 트리거**(SQLite 쓰기 경합·응답 지연)의 관측 근거라 배포와 함께 세팅 권장.
5. **(자격 확보 시 상시)** `git push origin main` — 현재 로컬 main이 origin보다 앞서 있음(미push). 인증되면 원격 동기화.

> 이 순서는 강제가 아니라 기본 권장안이다. 오너 우선순위에 따라 재배열 가능.
...
- [ ] **모니터링·에러 트래킹** — 프로덕션 로깅/에러 리포팅 도구 도입 검토. (3.2 착수 판단의 관측 근거가 되므로 함께 검토 권장.)
