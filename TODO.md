...
> 각 Phase가 독립된 큰 작업이므로 **한 화면군씩** 스코프를 잡아 진행(한꺼번에 몰면 검증이 흐려짐). **Phase 4~8 전부 완료(2026-07-17)** — `DESIGN_CHANGE_PLAN.md` 전 Phase(1~8) 개편 완료. 남은 것은 실기기 다중 뷰포트 시각 QA(계획서 §9, 위 🔵 「브라우저/시스템 테스트」와 동일)뿐이다.
...
- [x] **Phase 5 · 커뮤니티** ✅ **완료(2026-07-17 · code-reviewer APPROVED · 746 테스트 그린)** — 우수작 게시판(`board_posts` index/show·`_board_post` 카드 그리드·`_cheer_button`·`_sticker`)·토론방(`topics` index/show·`_forum_post`·`_like_button`) 8개 뷰를 디자인 시스템으로 개편. 목록은 반응형 카드 그리드(`sm:grid-cols-2 lg:grid-cols-3`), 상세는 읽기 폭(토론 `page-shell-reading`)·본문+보조패널(우수작 `content` 2컬럼), 응원·좋아요는 `.btn`(액션=`btn-yellow` 초대·완료=`btn-secondary`, 양 상태 `btn-sm` 동일 크기로 Turbo 갱신 시 레이아웃 이동 0). Turbo Stream(`dom_id(board_post,:cheer)`·`dom_id(forum_post,:like)`·`dom_id(@report,:stickers)` append·`dom_id(board_post)`·`dom_id(forum_post)`)·폼 필드명(sticker `position`/`emoji`/`label`, topic `title`/`scope`, forum_post `text`)·Pundit 게이트(`policy(Sticker.new).create?`·`policy(Topic).create?`)·`local_assigns.fetch(:liked)` 폴백 **전량 byte-보존**. 장식 이모지 `aria-hidden`, raw 팔레트 0, tailwind emit 실측. turbo_stream 3파일(cheers·forum_post_likes·stickers)은 partial 렌더만이라 무변경으로 새 스타일 자동 반영.
- [x] **Phase 6 · 역할별 심화 화면** ✅ **완료(2026-07-17 · 팀 4워커 + 공용 nav 선작업 · code-reviewer COMMENT[MUST-FIX 0]·targeted 테스트 그린)** — 교사 콘솔(반응형 `teacher/_nav`[데스크톱 사이드바+모바일 `<details>` 펼침, 라벤더/블루 톤]·대시보드·검토 큐·검토 상세·학생관리·미션/퀴즈/루브릭 CRUD)·사서(대시보드·인기대출·행사 CRUD, 민트/틸 톤)·교무(전교통계 넓은 표·NEIS 클립보드, 블루/틸 톤) **28개 뷰**를 디자인 시스템으로 개편. 콘솔 셸=`page-shell-wide`+`flex flex-col gap-6 lg:flex-row`(업무 데이터가 좁은 중앙열에 안 갇힘). 계약 **전량 byte-보존**: 검토큐 방송 타깃 `id="review_queue"`(리터럴)·`turbo_stream_from ..., :review_queue`·`_report_row` `dom_id(report)`(current_user 미참조)·퀴즈 중첩속성(`quiz_questions`/`_destroy`/`published`)·NEIS `clipboard` Stimulus(`data-action`/`data-clipboard-target`)·모든 `form_with`/`button_to` 라우트·필드명·정책 게이트. raw 팔레트 0, tailwind emit 실측, 데이터 표 `overflow-x-auto` 유지. 리뷰 SHOULD-FIX 2건 반영(사서 `text-brand-teal`[2.38:1 AA 미달]→`text-moss-dark`[5.48:1], 교무 로그아웃 `btn-primary`→`btn-secondary btn-sm` 역할 톤 통일). **인쇄 뷰(`teacher/prints/*`, `layout "print"`)는 Phase 8 대상이라 제외.**
- [x] **Phase 7 · 관리자 콘솔 CRUD** ✅ **완료(2026-07-17 · 팀 4워커 + 공용 pager 선작업 · code-reviewer MUST-FIX 0 · 760 테스트 그린)** — 총괄관리자 `/admin` 콘솔 **38개 뷰**(도서·상점아이템·뱃지·몬스터종·퀴즈·학교 각 index/show/new/edit/_form + 사용자 index/show/edit + 통합통계·시스템설정·모더레이션) + 공통 pager 2종을 디자인 시스템으로 개편. 오프캔버스 사이드바 셸(`layouts/admin`) 위 콘텐츠는 `page-shell` 없이 레이아웃의 `max-w-[96rem]` 폭·거터를 그대로 활용. `stat-card`·`overflow-x-auto` 데이터표·`form-*`·통일 pager 적용. **파괴적(삭제·계정 정지·숨기기)=`btn-danger` vs 복원적(정지 해제·숨김 해제)=`btn-secondary` 시각 분리(Phase 7 목표).** 계약 **전량 byte-보존**(폼 라우트·필드명·퀴즈 `fields_for :quiz_questions` 중첩[prompt/choices/answer_index/position/_destroy/published]·`button_to`+`turbo_confirm`·pager 로컬[page/has_next/param]·검색 params[q/role/school_id]·analytics `@school_stats`/`RUBRIC_AXES`·settings `JSON.pretty_generate`·moderation `kind:`) — 계약 토큰 HEAD 대조 불일치 0. raw 팔레트 0. 리뷰 SHOULD-FIX 3건(대비 회귀: 정답색 `text-success-accent`→`text-success-ink`, 표창장 캡션 `text-brand-yellow-deep`→`text-yellow-dark`, 모더레이션 작성자명 `text-muted`→`text-steel`) + 일관성 NIT(목록 행 액션 텍스트링크 통일·폼 오류배너 `role="alert"` 통일) 반영.
- [x] **Phase 8 · 부가 표면** ✅ **완료(2026-07-17 · code-reviewer 승인 · 760 테스트 그린)** — 인쇄 레이아웃(`layouts/print` 툴바 `btn`[화면 전용, `.no-print`]·`@media print`/`@page`/`window.print()` 규칙 보존)·교사 인쇄물 4종(표창장·학급성장리포트·가정통신문·독서포트폴리오 — **흑백 인쇄 가독 우선**, `radar_chart_svg`·`axis_label`·growth-card Stimulus[`data-controller`·`data-growth-card-*`·canvas 320×220·`.no-print`] 전량 보존)·메일러 HTML 레이아웃(이메일 세이프 **인라인 스타일** 브랜드 셸 + `yield`)·PWA `manifest.json.erb`(플레이스홀더 `"red"`→브랜드 `#ffd02f`/`#ffffff`, 이름 `BookmarkApp`→`책갈피`)·잔여 turbo_stream(`purchases` 상점 flash raw 팔레트→`state-banner` 토큰) 정리. Turbo·인쇄·메일·PWA 동작 회귀 0. **→ `DESIGN_CHANGE_PLAN.md` 전 Phase(1~8) 개편 완료.**
