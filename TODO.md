...
- [ ] **재롤·워밍 rate limit/예산 수치 튜닝** — `REGENERATE_PER_USER`(10/h)·`WARMING_PER_USER`(20/h)·`WARMING_DAILY_BUDGET`(500/day). **실운영 텔레메트리 없이는 재산정 불가** → 아래 품질·운영의 모니터링 도입 후 실사용 분포로 튜닝.
- [ ] **무게이트 롤아웃 정책** — 파일럿 학급 선정·확대 기준, 신고 자동 숨김 임계, 교사 opt-in 사후검토 시점. **오너 정책+파일럿 관측 필요** → 이미 구현된 스코프형 kill switch/피처플래그로 파일럿부터 시작해 결정.
