...
- 권한: 인터넷 하나 (카메라·저장소 권한을 요청하지 않습니다)
- 카메라: 선택 (손글씨 사진 인식에 쓰며, 카메라가 없으면 앨범에서 사진을 고릅니다)

[권장 사양]
- 10인치급 태블릿: Galaxy Tab S6 Lite(SM-P610) 이상
- 가로·세로 화면 모두 지원

※ Play 스토어가 아닌 APK 파일로 설치하므로 '출처를 알 수 없는 앱 설치 허용'이 필요합니다.
