...
# P2-3 몬스터 성장 서사 로더(docs/monsters.md §5.7 → db/seeds/monster_stories.yml).
...
class MonsterLoreTest < ActiveSupport::TestCase
  ...
  test "for returns the story for a known dex and nil for an unknown one" do
    ...
    assert_equal "처음 만남", story.scene_for(1).title
    assert_nil story.scene_for(4)
    assert_nil MonsterLore.for(999)
  end

  test "growth messages match docs/monsters.md §5.7 (unified to 해요체) at both endpoints" do
