...
class StudentHeaderProfileTest < ActionDispatch::IntegrationTest
  ...
  def assert_student_header
    ...
    assert_select "header.app-header a.btn.btn-primary[href='#{profile_path}']", text: /마이페이지/, count: 1
    assert_select "header.app-header form[action='#{session_path}'] button.btn.btn-primary", text: /로그아웃/, count: 1
  end
end
