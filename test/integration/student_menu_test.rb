...
class StudentMenuTest < ActionDispatch::IntegrationTest
  ...
  # 모든 필터가 같은 책 카드다(2026-09-20) — 책 소개·뒷이야기·토론 글은 책 제목 → 이 책의 내 기록에서 본다.
  test "책 소개·뒷이야기·토론 필터도 책 카드로 보이고 남의 글·숨김 글로는 책이 오르지 않는다" do
    other = Book.create!(title: "두 번째 책", author: "지은이", category: :recommended)
    peer_book = Book.create!(title: "친구만 쓴 책", author: "지은이", category: :recommended)
    hidden_book = Book.create!(title: "숨긴 글만 있는 책", author: "지은이", category: :recommended)
    ...
    BookSequel.create!(user: peer, book: peer_book, classroom: @classroom, body: "친구가 쓴 뒷이야기예요.")
    ...
    hidden_topic = Topic.create!(scope: :classroom, classroom: @classroom, book: hidden_book, title: "숨긴 토론")
    ForumPost.create!(topic: hidden_topic, user: @student, text: "숨긴 내 토론 글", hidden: true)

    get library_path(kind: "sequel")
    assert_select "a[href=?]", library_book_path(@book), text: "메뉴책"
    assert_select ".badge", text: "뒷이야기 이어쓰기"
    assert_no_match "첫 번째 책의 뒷이야기예요", response.body
    assert_no_match "친구만 쓴 책", response.body

    get library_path(kind: "book")
    assert_select "a[href=?]", library_book_path(other), text: "두 번째 책"
    assert_no_match "메뉴책", response.body

    get library_path(kind: "forum")
    assert_select "a[href=?]", library_book_path(@book), text: "메뉴책"
    assert_select ".badge", text: "토론 글 1"
    assert_no_match "내 토론 글이에요", response.body
    assert_no_match "숨긴 글만 있는 책", response.body
  end

  test "고른 활동을 한 책이 없으면 책이 없다는 안내를 보인다" do
    %w[forum book sequel].each do |kind|
      get library_path(kind:)
      assert_match "이 활동을 한 책이 아직 없어요", response.body, "kind=#{kind}"
    end
  end
