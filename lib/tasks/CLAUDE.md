...
- `books.rake` — 네 태스크. **`books:seed`**는 호환 태스크명으로 ISBN이 검증된 TSV가 있으면 `books:seed_full`에 위임하고, 없으면 제목-only 카탈로그를 만들지 않고 no-op 한다. **`books:seed_full`**은 `db/seeds/elementary_books.tsv`를 오프라인·멱등 적재하며 ISBN 공란·체크디지트 오류 행을 각각 집계해 제외하고, 정규화 ISBN-13으로 동일 `searched` 행까지 제자리 승격한다. TSV 값이 있는 메타만 비파괴 갱신하고 summary는 보존한다(`ENV["BOOKS_TSV"]`로 테스트 파일 주입). **`books:enrich`**는 기존 ISBN 정확 일치 결과로 표지·출판사만 수동 네트워크 보강한다. **`books:deduplicate_isbn`**은 제약 도입 전 레거시의 정규화 ISBN 중복과 고신뢰 공란-ISBN 그림자를 기본 dry-run으로 보고하고, `APPLY=1`일 때만 참조 이관·메타 병합·중복 삭제한다(`INCLUDE_SHADOWS=0`으로 ISBN 중복만 제한, system quiz 충돌은 skip).
...
- `games.rake` — `games:warm`(Phase 3 §3.5, A5). 카탈로그(추천/고전) 도서를 대상으로 `Games::ContentProvider.warm!` 로 **band×content_axis 워밍 잡을 사전 큐잉**해 학생 첫 플레이의 콜드-첫-오프라인 노출(특히 hint_reveal)을 줄인다(게임 재구성 Phase 1: matching 생성 경로 제거로 mcq·hint_reveal 만 워밍). 예산·rate limit·스코프 플래그를 준수하고 무키면 아무것도 적재하지 않으며(오프라인만), dedup(이미 워밍 중/AI 게시됨)은 `GenerateGameContentJob` 이 자체 가드한다. 시드 아닌 운영 태스크(스케줄/도서 지정 시 수동 실행).
