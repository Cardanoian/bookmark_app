...
- `books.rake` — 세 태스크. **`books:seed`**(TSV가 없는 체크아웃용 축소 큐레이션: 초등 1~2/3~4/5~6 권장도서 + 고전, `title`+`author` 멱등). **`books:seed_full`**(`db/seeds/elementary_books.tsv` 8,502행을 오프라인·멱등 적재; `isbn13` 우선, 없으면 `title`+`author`; **isbn 있는 행은 동일 isbn `searched` 캐시 행까지 포함해 제자리 승격**[비-scoped `find_or_initialize_by(isbn:)` → category 만 카탈로그로 승급, id·isbn 보존으로 `reports.book_id` 링크 보존; 부분 유니크 인덱스 하 동일 isbn 공존 불가라 삭제 대신 승격]; isbn 없는 행만 `searched` 제외 title+author dedup; TSV 값이 있는 메타만 비파괴 갱신하고 summary 보존). TSV가 있으면 `db:seed`의 기본 경로이고 없으면 `books:seed`로 폴백하며, `ENV["BOOKS_TSV"]`로 테스트 파일 주입 가능. **`books:enrich`**는 seeds 밖 수동 네트워크 보강이다.
