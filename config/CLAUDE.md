...
- **학생 영역(최상위)**
  ...
  - 계정 — `profile/password`(GET passwords#edit / PATCH passwords#update, `edit_password_path`/`password_path`) 본인 비밀번호 변경(역할무관, 현재 비번 확인 후 변경). **`account_link`(singular, `account_links` 컨트롤러, account_linking_seasons_plan §Phase 3)**: `new`(GET `/account_link/new`, `new_account_link_path`) + `preview`(POST `/account_link/preview`, `preview_account_link_path`) + `confirm`(POST `/account_link/confirm`, `confirm_account_link_path`) — 학생이 작년 계정 자격증명을 증명해 병합(플래그 `account_linking` 게이트).
- **역할별 네임스페이스** (컨트롤러 `Xxx::BaseController` 가 자기 학교/권한 경계를 가드)
  ...
  - `namespace :admin` — 총괄관리자(superadmin) 전용. root=`analytics#show`, 전 자원 관리(`schools`·`users`·`books`·`recommendation_imports`[XLSX index/create]·`quizzes`·`badges`·`monster_species`·`moderation`·**`game_contents`[index/destroy + member `hide`·`restore` — 전국 노출 system 풀 퀴즈의 신고 콘텐츠 에스컬레이션, Phase 3 §4.5]**·**`account_links`[index/create + member `reverse` — 계정 연동 총괄 보조·되돌리기(시간창 무제한), account_linking_seasons_plan §Phase 4. 라우트만 선점, 컨트롤러는 Phase 4 미착수]**·`settings`·`analytics`). `Admin::BaseController` 가 superadmin 외 전 역할 403.
