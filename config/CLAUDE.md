...
- **학생 영역(최상위)**
  ...
  - 게임화 — `monsters`(도감/`evolve`·`set_active`·`choose_starter`), `rankings`, **`challenges`(전체 CRUD + member `join` — 관리[`new`/`create`/`edit`/`update`/`destroy`]는 교직원 전용, `scope`·`school_id`는 폼 입력이 아니라 역할에서 파생[위조 차단])** + `discoveries/acknowledge`(POST, `acknowledge_discoveries_path`) 몬스터 발견 연출 확인 후 celebrated_at 마킹(재노출 방지). **미션은 자동 배정·자동 진행이라 학생 `missions`·`join` 라우트가 없다**(menu_refactor 심화 PR6 — 홈·독서활동 문맥 카드로 노출). **상점(`shop`·`purchases`)·먹이(`feed`)는 PR7/8 에서 제거**.
