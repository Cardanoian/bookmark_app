...
- **학생 영역(최상위)**
  ...
  - **`library`**(singular, `libraries#show`) — 내 서재(책별 활동 포트폴리오) · **`reading_activity`**(singular, `reading_activities#show` + 중첩 `get :nearby_libraries`) — 독서활동 허브(책 선택→독후감/게임). **`nearby_libraries`(`nearby_libraries_reading_activity_path(book_id:)`, 단수 헬퍼라 positional 인자 금지)**는 활동 화면 아래 인근 도서관 대출 가능 섹션의 Turbo Frame lazy-load 전용 액션(정보나루 `libSrchByBook`+`bookExist`, 동일출처 프레임이라 CSP 변경 불필요). 학생 상위 메뉴는 홈·내 서재·독서활동·도감·나의 성장이고 랭킹은 성장 화면의 선택 경로다.
  - `namespace :games` — 독서게임 4종(Phase 3 온디맨드 + 소셜; 게임 재구성 Phase 1·2 로 정리 — quiz·whoami·book·sequel). **카탈로그** `games/catalog`(도서→게임 진입 관문) + **퀴즈 파이프라인 2종 표면의 `<표면>/play`**(book_id 온디맨드 진입; quiz·whoami — classic[→quiz 통합]·vocab[hard-delete] 표면 제거) + quiz 의 교사 published mcq 퀴즈 `:id` show 병행 + whoami `:id` show(=attempt 상태) 와 **`whoami/:attempt/reveal_hint`**(POST, 서버 힌트 공개) + **책 소개 대결** `book/play`(GET, 도서별 소개 목록·작성 폼)·`book/intros`(POST 작성)·`book/intros/:id/vote`(POST/DELETE 또래 1인 1표) + **뒷이야기 이어쓰기** `sequel/play`(GET, 도서별 뒷이야기 목록·작성 폼)·`sequel/entries`(POST 작성)·`sequel/entries/:id/vote`(POST/DELETE 또래 1인 1공감; 제출 시 `SequelFeedbackJob`이 학생 글 격려 AI 코멘트 비동기, 무API 폴백) + **`regenerate`**(POST, 다시 뽑기=콘텐츠 재생성; 무가챠 라우트 가드 준수차 `roll` 어휘 회피) + **`content_reports`**(POST, 콘텐츠 신고=무게이트 롤아웃 안전장치; system 판 `quiz_id`, 서로 다른 2명 시 자동 숨김+재생성) + 결과 기록용 `attempts#create`(퀴즈 2종). `play/…play` 라우트를 `:id` show 보다 먼저 선언해 "play" 가 id 로 오인되지 않게 한다. book 은 퀴즈 파이프라인 밖 소셜 도메인(Gemini/Quiz 미생성).
  - 커뮤니티 — `board_posts`(우수작, 중첩 `cheers`·`stickers`) + `topics`(토론방, 중첩 `forum_posts`) + `forum_posts` 단수 `like`(좋아요)·`report`(신고, reading_discussion — 1인 1신고, 자동숨김 없음). 교사 토론 모더레이션은 `namespace :teacher` 의 `forum_posts/:id/hide`·`/unhide`(저자 학급 담임만).
  - 게임화 — `monsters`(도감/`evolve`·`set_active`·`choose_starter`), `rankings`, **`challenges`(전체 CRUD + member `join` — 관리[`new`/`create`/`edit`/`update`/`destroy`]는 교직원 전용, `scope`·`school_id`는 폼 입력이 아니라 역할에서 파생[위조 차단])** + `discoveries/acknowledge`(POST, `acknowledge_discoveries_path`) 몬스터 발견 연출 확인 후 celebrated_at 마킹(재노출 방지). **미션은 자동 배정·자동 진행이라 학생 `join` 라우트는 없으나, 카드→상세 이동을 위해 열람 전용 `resources :missions, only: [:show]`(최상위, `mission_path`, `missions#show`)를 둔다**(challenge-mission-detail — 홈 미션 카드 클릭 시 상세[상단 정보+하단 책목록→reading_activity]로 이동; 관리 CRUD 는 `namespace :teacher` 유지. PR6 의 "미션 독립 화면 없음"을 사용자 요구로 열람 한정 확장). 학생 상세 인가는 `MissionPolicy#show?` 가 학급 경계로 강제. **상점(`shop`·`purchases`)·먹이(`feed`)는 PR7/8 에서 제거**.
  - 계정·성장 — `profile/password`(GET passwords#edit / PATCH passwords#update, `edit_password_path`/`password_path`) 본인 비밀번호 변경(역할무관, 현재 비번 확인 후 변경). **`growth`(singular show)**는 승인 독후감 5축 자기비교 화면, **`ranking_preference`(singular edit/update)**는 학생 닉네임·랭킹 참여 선택 화면. **`account_link`(singular, `account_links` 컨트롤러, account_linking_seasons_plan §Phase 3)**: `new`(GET `/account_link/new`, `new_account_link_path`) + `preview`(POST `/account_link/preview`, `preview_account_link_path`) + `confirm`(POST `/account_link/confirm`, `confirm_account_link_path`) — 학생이 작년 계정 자격증명을 증명해 병합(플래그 `account_linking` 게이트).
- **역할별 네임스페이스** (컨트롤러 `Xxx::BaseController` 가 자기 학교/권한 경계를 가드)
  - `namespace :teacher` — 담임교사. `dashboard`, 검토 큐 `reviews`(`approve`·`verify`·`batch_approve`), `students`(`reset_password`·`give_points`), `missions`·`quizzes`, **`quiz_contributions`(index/update + member `approve`·`reject` — 학생 기여 문제 검토·수정 큐, 승인 시 전국 공유 풀 물질화, Phase 3 §4.3)**, `rubric_config`, **`account_links`(index/new/create + member `reverse` — 계정 연동 교사 보조·되돌리기, account_linking_seasons_plan §Phase 4. 라우트만 선점, 컨트롤러는 Phase 4 미착수)**, 문서출력(`exports#reports_csv` + `prints` 의 표창장·가정통신문·포트폴리오·학급리포트).
  ...
  - `namespace :admin` — 총괄관리자(superadmin) 전용. root=`analytics#show`, 전 자원 관리(`schools`·`users`·`books`·`recommendation_imports`[XLSX index/create]·`quizzes`·`badges`·`monster_species`·`moderation`·**`audit_logs`[index — 상태변경/CSV 감사 원장 필터·페이지네이션, 쓰기 라우트 없음]**·**`game_contents`[index/destroy + member `hide`·`restore` — 전국 노출 system 풀 퀴즈의 신고 콘텐츠 에스컬레이션, Phase 3 §4.5]**·**`account_links`[index/create + member `reverse` — 계정 연동 총괄 보조·되돌리기(시간창 무제한), account_linking_seasons_plan §Phase 4]**·`settings`·`analytics`). `Admin::BaseController` 가 superadmin 외 전 역할 403.
...
- `recurring.yml` — Solid Queue 반복 작업(production). 매시 완료 잡 정리 + **미션 완료·보상 주기 재평가**(`Missions::ReevaluateJob`, menu_refactor 심화 §2.A.6 안전망2 — 이벤트 훅 미스·지연배정 갭 백스톱, Rewarder 멱등이라 재실행 안전) + **계정 연동 자격증명 야간 purge**(`Accounts::PurgeCredentialsJob` `at 4am every day`, account_linking_seasons_plan §Phase 5 — 교사 되돌리기 창[14일] 경과·미되돌림 병합 snapshot 의 password_digest nullify, PII 보존기간 제한·멱등. 잡 클래스로 스케줄해 rake 의존 회피). **읽기 전용 리포트(`account_links:audit`·`seasons:reconcile`)는 로그 스팸을 피하려 recurring 미등록** — 운영자가 온디맨드/외부 스케줄러로 실행. dev/test 는 production 전용이라 즉시 평가 + 수동 rake 로 커버.
...
- **스코프형 기능 플래그 / kill switch(Phase 2b C3)**: 온디맨드 게임 콘텐츠 워밍은 `AppSetting.feature_enabled?("on_demand_games", scope:)` 로 제어한다(설정 값은 `app_settings` 의 `feature_flags` JSON, 관리자 `admin/settings`). 저장 규약 — `"on_demand_games"` 전역 값(**false=하드 kill: 스코프 무시·전부 오프라인**, 미설정=파일럿[기본 off, 스코프 on 만], true=확대[기본 on, 스코프 off 로 개별 격리]) + `"on_demand_games:classroom:<id>"`/`":school:<id>"` 스코프 오버라이드(학급 우선→학교). **한 학급 사고를 전교 off 없이 격리**하고 파일럿→확대 롤아웃을 가능케 한다(교사 검수 게이트 부활 아님, R4 준수). 기본 seeds 는 `on_demand_games => true`. rate limit/예산은 `RateLimiter`(Solid Cache 원자 increment)가 담당. **독서 토론도 같은 규약을 재사용**: `reading_discussion` 플래그가 토론 컨트롤러(`topics`·`forum_posts`·`forum_post_reports`)를 게이트하고(뷰 은닉만으론 URL 직접 요청 불가), 안전 스택(신고·모더레이션·금칙어)을 함께 출하하므로 **미설정 기본=활성**(`feature_enabled?("reading_discussion", default: true)`, 기본 seeds `reading_discussion => true`). 전역 false=하드 kill, `reading_discussion:classroom:<id>`=false 로 학급 격리. **랭킹 시즌제도 같은 규약을 재사용(account_linking_seasons_plan §Phase 1)**: `ranking_seasons` 플래그(뷰어 학급 스코프)가 `RankingBoard`·랭킹 뷰(`_ranking_row`/`_podium`)의 **읽기 경로만** 게이트해 정렬·표시 기준을 평생 `users.experience`↔현재 학년도 `season_scores.experience_earned` 사이에서 전환한다(**미설정 기본=비활성** `feature_enabled?("ranking_seasons", default: false)`, **기본 seeds `ranking_seasons => false`**[Phase 5 additive] — 전환 스위치라 2027-03 학년도 경계에 전역 on 예정, 2026 시즌은 부분·비권위). **시즌 데이터 축적(Pointable 훅)은 플래그와 무관하게 항상** 동작하므로 플래그를 켜기만 하면 즉시 시즌 랭킹이 활성화된다. **계정 연동도 같은 규약(account_linking_seasons_plan §Phase 3)**: `account_linking` 플래그(뷰어 학급 스코프)가 `AccountLinksController`(before_action `require_account_linking!`)와 마이페이지 진입점(`ProfilesController#account_link_available?`)을 게이트한다. 구현 완료 기능이므로 **미설정 기본=활성**(`feature_enabled?("account_linking", default: true)`, 기본 seeds `account_linking => true`)이며, 전역 false는 하드 kill, `account_linking:classroom:<id>`/`:school:<id>`=false는 범위별 격리에 사용한다.
