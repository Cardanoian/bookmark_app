...
## 8. 사용자 결정 (확정)
1. **`ranking_seasons` on 타이밍 → 2027-03 학년도 경계.** 2026 부분 시즌은 비권위(집계만), 2027학년도부터 시즌 랭킹 활성.
2. **데이터 이전 동의 → 별도 동의 게이트 없음.** 확인 화면(이동 자산 명시) + 비번 증명(셀프서브)/교사 소유(폴백)로 갈음.
3. **되돌리기 창 → 교사 14일 · 총괄 무제한(수용).** 교사는 `TEACHER_REVERSE_WINDOW`(14일) 내, 총괄은 제한 없음. 교사 창을 14일로 유지하는 근거: 자격증명 purge 잡이 이 창 경과 후 snapshot의 `password_digest`를 nullify하므로 삭제 아동 계정 비번 해시의 무기한 보존을 막는다(보안 리뷰 PII 완화). 총괄 무제한 reverse 는 purge 후 구조 복원은 가능하되 복원 placeholder 로그인엔 담임 비번 재설정이 필요하다.

### 구현 상태 (2026-07-21 team 실행 완료)
전 Phase(0~5) + 리뷰 fix 2배치 main 커밋(`a19e862`~`75b0d85`). 독립 검증: 테스트 1380 runs 0 실패 · RuboCop 무위반 · Brakeman 0. 두 기능은 플래그(`account_linking`·`ranking_seasons`) 기본 **off**라 휴면 — `admin/settings`에서 켜야 활성.
