...
현재 저장소에는 `ApplicationMailer`의 기본 틀만 있고 실제 메일러 클래스나 발송 호출이 없다. `users` 테이블에는 이메일 컬럼(유니크 인덱스)이 있어 교사·학교관리자·사서·총괄관리자는 이미 이메일로 로그인하지만, 학생은 이메일 없이 튜플 로그인을 쓴다. 학생 비밀번호는 교사·관리자가 임시 비밀번호로 초기화해 화면에서 전달한다.
