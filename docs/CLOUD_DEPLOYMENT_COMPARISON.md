...
- `users` 테이블에 이메일 컬럼(교직원 로그인 식별자, 유니크 인덱스)은 있으나 실제 이메일 발송 기능은 없음
...
현재 저장소에는 `ApplicationMailer`의 기본 틀만 있고 실제 메일러 클래스나 발송 호출이 없다. `users` 테이블에는 이메일 컬럼(유니크 인덱스)이 있어 교사·학교관리자·사서·총괄관리자는 이미 이메일로 로그인하지만, 학생은 이메일 없이 튜플 로그인을 쓴다. 학생 비밀번호는 교사·관리자가 6자 이상으로 직접 지정해 초기화하고 별도 경로로 학생에게 전달한다.
...
`users` 테이블의 이메일 컬럼은 교직원 로그인 식별자로 이미 쓰이고 있으므로, 여기서는 컬럼 신설이 아니라 성인 역할의 이메일 저장 정책(역할별 필수 여부·검증 범위)과 실제 발송 파이프라인(메일러 클래스·템플릿·Resend 연동) 추가가 과제다.
