...
module ReadingDomain
  ...
  AXIS_MEANINGS_BY_BAND = {
    ...
    g56: {
      content: "인물·사건·배경 파악",
      emotion: "생각과 느낌의 구체성",
      life: "자신의 삶·경험과의 연관과 성찰",
      structure: "문장 연결·구성·고쳐쓰기",
      spelling: "표기 정확성"
    }
  }.freeze

  # 축별 점수 앵커(5·3·1점). 학년군 무관 — 판정 기준이 "글에 무엇이 있는가"라서 눈높이는
  # 앞선 학년군 제약(성취기준 allowlist·tone)이 이미 상한을 잡는다.
  #
  # ⚠️ 이 상수의 존재 이유 — 등급 인플레이션 차단. 앵커 없이 "0~5로 채점하라"만 주면 LLM 은
  # 거의 모든 축에 4를 준다(실측: Gemini 축평균 4.37·맞춤법 축 표준편차 0.00[7건 전부 4점],
  # Haiku 4.11). 5축 평균이 LEVEL_A_MIN_AVG 를 넘으면 RubricScorable#level_for 가 A 를 주므로,
  # 앵커가 없으면 사실상 전원 A 가 되어 등급·포인트·몬스터 진화 인센티브가 통째로 평탄해진다.
  # 기본값을 3(기대 수준)으로 못 박고 4·5 를 "본문에서 근거를 찾았을 때만"으로 제한하는 것이
  # 이 상수의 핵심 역할이다. 성취기준 코드는 절대 넣지 말 것(학년군 누출 테스트가 막는다).
  RUBRIC_SCORE_ANCHORS = {
    content: {
      5 => "인물·사건·배경을 정확히 짚고, 이야기의 흐름을 원인과 결과로 이어 정리했다",
      3 => "중심 내용은 맞게 썼으나 일부가 빠지거나 순서가 뒤섞였다",
      1 => "책 내용을 거의 옮기지 못했거나 사실과 다르게 썼다"
    },
    emotion: {
      5 => "어느 장면에서 왜 그렇게 느꼈는지 까닭을 들어 자기 말로 썼다",
      3 => "느낌을 썼지만 '재미있었다·감동적이었다' 같은 한 마디에 머문다",
      1 => "느낌이라 할 만한 표현이 없다"
    },
    life: {
      5 => "자신이 겪은 구체적인 일을 들어 책과 견주고, 그로 인해 생각이나 행동이 달라진 지점을 밝혔다",
      3 => "다짐이나 교훈을 한 문장으로 적었으나 자신의 경험은 들지 않았다",
      1 => "자신과 연결한 내용이 없다"
    },
    structure: {
      5 => "처음·가운데·끝이 나뉘고 문장 연결이 자연스러우며 한 덩어리가 한 가지를 말한다",
      3 => "흐름은 이어지지만 문단 구분이 없거나 문장이 길게 늘어진다",
      1 => "문장이 끊기거나 순서가 뒤엉켜 읽기 어렵다"
    },
    spelling: {
      5 => "맞춤법·띄어쓰기에서 고칠 곳을 찾지 못했다",
      3 => "고칠 곳이 두세 군데 있으나 뜻을 알아보는 데 지장이 없다",
      1 => "고칠 곳이 많아 문장을 읽기 어렵다"
    }
  }.freeze

  # 축별 2022 개정 국어 성취기준 "대표" 코드(학년군별). 5축 각각에 가장 잘 맞는 대표 코드 1개다.
  # 원문은 각 코드 옆 주석 참고. 코드 근거: 교육부 고시 제2022-33호 [별책 5] 국어과 교육과정
  # (원문 대조 출처는 CURRICULUM_STANDARDS_BY_BAND 주석).
  ...
  #
  # 매핑 규칙 — 코드 번호가 아니라 **원문 문장의 뜻**이 그 축과 맞아야 한다. 2026-09 원문 대조 전에는
  # 설명이 원문과 달라서 g34 의 content·life 가 뒤바뀌어 있었다(원문 [4국05-01]=인물과 이야기의 흐름,
  # [4국05-02]=경험 바탕 작품 속 세계와 현실 세계 비교). g56 spelling 도 문장 성분 호응([6국04-04])
  # 대신 단어·문장·띄어쓰기를 바르게 고치는 [6국04-06](원래 목록에서 빠져 있었다)을 가리키도록 고쳤다.
  ACHIEVEMENT_STANDARDS_BY_BAND = {
    ...
    g34: {
      ...
      emotion: "[4국05-04]",   # 감각적 표현에 유의하여 작품을 감상하고, 감각적 표현을 활용하여 자신의 생각이나 감정을 표현한다.
  ...
  # 앱 안 사본: app/views/monsters/성취기준.md(이 상수와 같은 문장을 유지). 독후감 첨삭과 직접 관련된
  # 읽기(02)·쓰기(03)·문법(04)·문학(05) 영역만 담는다(듣기·말하기(01)·매체(06)는 독후감 첨삭과 무관해 제외).
  # 첨삭 프롬프트(build_rubric_prompt)가 "이 목록의 코드만 인용하라"는 하드 제약의
  # 근거(allowlist)로 주입해, LLM 이 상위 학년 성취기준을 학생에게 요구하지 못하게 막는다.
  #
  # ⚠️ 문장은 고시 원문 그대로다 — 요약·의역 금지. 2026-09-13 원문 대조 결과 이전 값은 3~4·5~6학년군에서
  # 23개 문장이 원문과 달랐고(예: [4국05-01]·[4국05-02] 뜻이 서로 뒤바뀜) [6국04-06]이 빠져 있었다.
  # 대조 원천(두 곳의 초등 성취기준 87개 문장이 가운뎃점 표기[⋅/·]만 빼고 모두 일치):
  #   ① 교육부 고시 게시글 "[교육부 고시 제2022-33호] 초중등학교 교육과정 총론 및 각론 고시"(2022-12-22)
  #      붙임2 zip 안의 "[별책5] 국어과 교육과정.hwp"
  #      https://www.moe.go.kr/boardCnts/viewRenew.do?boardID=141&lev=0&statusYN=W&s=moe&m=0404&opType=N&boardSeq=93458
  #   ② 같은 고시 원문 PDF(가톨릭관동대 국어교육과 게시)
  #      https://www.cku.ac.kr/bbs/korean/907/167753/artclView.do
  # 가운뎃점은 저장소 관례대로 "·"(U+00B7)로 적는다(원문 HWP 는 "⋅" U+22C5).
  ...
  # 학년군별 프롬프트 메타(대상 라벨·어조). 프롬프트 빌더가 참조.
  # tone 은 모델에게 주는 지시라 "~하세요" 체다. 학생에게 쓰는 문장의 말투(해요체·호칭 금지)는
  # CHILD_VOICE_RULES, 제안 개수는 FEEDBACK_LIMITS_BY_BAND 가 정하며 tone 은 그와 어긋나면 안 된다
  # (g12 "고칠 점은 딱 한 가지만" = fix 상한 1). tone 은 퀴즈 콘텐츠 프롬프트(build_content_prompt)에도 들어간다.
  PROMPT_META = {
    g12: {
      grade_label: "초등학교 1~2학년",
      tone: "이제 막 글쓰기를 배우는 1~2학년입니다. 아주 쉽고 다정한 말로, 잘한 점을 먼저 크게 칭찬하고 고칠 점은 딱 한 가지만 부드럽게 제안하세요. 어려운 낱말이나 평가 용어는 쓰지 마세요."
    },
    g34: {
    ...
    g56: {
      ...
      tone: "스스로 고쳐 쓸 수 있는 5~6학년입니다. 맞춤법만 고치지 말고 내용·감상·삶과의 연결·구성·표기를 두루 살피되, 제안은 가장 중요한 한두 가지로 좁혀 쉬운 말로 주세요."
    }
  }.freeze

  # 학년군별 첨삭 제안 개수 상한. 베타 리뷰에서 "한 번에 고칠 게 너무 많다"는 지적이 나와, 학생이
  # 한두 가지에 집중하도록 묶는다. g12 는 tone 의 "고칠 점은 딱 한 가지만"과 맞춰 fix 1개.
  # 프롬프트([제안 개수와 순서])와 Ai::ReviewService#normalize(모델이 넘겨도 초과분 절삭)가 같은 값을 쓴다.
  FEEDBACK_LIMITS_BY_BAND = {
    g12: { fix: 1, grow: 1 },
    g34: { fix: 2, grow: 1 },
    g56: { fix: 2, grow: 1 }
  }.freeze

  # ── 학생 대면 AI 문장 규칙(베타 리뷰 지적 사항 대응) ────────────────────────────────
  # 아래 상수는 학년군 무관이라 세 밴드 프롬프트에 모두 들어간다. 그러므로 성취기준 코드를 절대
  # 넣지 말 것(학년군 누출 가드 테스트가 막는다 — RUBRIC_SCORE_ANCHORS 와 같은 이유).
  # 예시 속 따옴표는 작은따옴표만 쓴다 — 모델이 큰따옴표를 흉내 내면 JSON 문자열이 깨져
  # ClaudeClient 가 파싱에 실패하고 규칙기반 폴백으로 떨어진다.

  # 무의미 입력 게이트(첨삭). 자모 나열·반복·책과 무관한 글에도 5축 칭찬·C등급·성취기준 코드가
  # 붙던 문제 대응. 짧거나 맞춤법이 서툰 저학년 글·OCR 의 '?' 표시를 오판하지 않게 예외를 함께 둔다.
  REVIEW_NON_ATTEMPT_RULES = <<~RULES.freeze
    [먼저 확인 — 책을 읽고 쓴 글인가요?]
    - 채점하기 전에, 이 글이 그 책을 읽고 내용이나 생각·느낌을 쓰려고 한 글인지 먼저 확인하세요.
    - 다음은 독후감이 아닙니다: 자음·모음만 늘어놓은 글(예: 'ㄱㅀㅀ ㅀㅇㄹ'), 같은 글자나 낱말만 되풀이한 글(예: '테스트테스트테스트', 'ㅋㅋㅋㅋ'), 뜻 없이 아무 글자나 누른 글, 광고나 다른 주제의 글처럼 책과 분명히 관계없는 글.
    - 독후감이 아니면 칭찬하지 마세요. praise 와 grow 는 빈 배열 [] 로 두고, rubric 다섯 축은 모두 0점, level 은 "C" 로 하세요. fix 에는 책을 읽고 다시 써 달라는 짧고 다정한 부탁 한 문장만 넣으세요(예: '책에서 기억에 남는 장면과 그때 든 생각을 두세 문장으로 다시 써 줄래요?').
    - 짧거나 맞춤법이 많이 틀려도 책에 대해 쓰려고 한 흔적이 있으면 독후감입니다. 이런 글은 아래 규칙대로 평소처럼 첨삭하세요.
    - 사진에서 옮긴 글에는 알아보지 못한 글자가 '?'로 표시돼 있을 수 있습니다. 그 자리는 학생 잘못이 아니니 지적하지 마세요.
  RULES

  # 학생 글 근거 규칙(첨삭). 이미 마침표로 나뉜 두 문장을 인용하고 "세 번째 문장이 기니 나누라"고
  # 해, 이미 충족된 변경을 요구하던 문제 대응 — 원문 그대로 인용 + 지적 전 자기 점검 + 동일 제안 삭제.
  REVIEW_GROUNDING_RULES = <<~RULES.freeze
    [학생 글에 근거하기 — 그대로 인용하고, 고치자고 하기 전에 한 번 더 확인하기]
    - 글의 한 부분을 칭찬하거나 고치자고 할 때는 그 부분을 작은따옴표(' ') 안에 학생이 쓴 그대로 옮기세요. 낱말·띄어쓰기·맞춤법·문장부호를 한 글자도 바꾸지 말고, 떨어져 있는 문장을 하나로 이어 붙이지도 마세요. JSON 이 깨지지 않도록 큰따옴표는 쓰지 마세요.
    - 칭찬할 때도 똑같습니다. 학생이 틀리게 쓴 낱말은 틀린 그대로 옮기세요(예: 학생이 '도와주고 십다'라고 썼으면 '도와주고 싶다'로 바꿔 인용하지 않기). 고친 모양은 fix 에서만 보여 주세요.
    - '세 번째 문장'처럼 순서로 가리키지 말고 인용으로 가리키세요.
    - fix 를 쓰기 전에, 인용한 부분에 정말 그 문제가 있는지 한 번 더 확인하세요.
      · 문장을 나누자고 할 때: 인용한 부분이 마침표 없이 길게 이어진 한 문장일 때만 권하세요. 이미 마침표(.)로 나뉜 문장이면 나누자고 하지 마세요.
      · 맞춤법·띄어쓰기를 고치자고 할 때: 그 낱말이 정말 틀렸을 때만 권하고, 인용 뒤에 바르게 고친 모양을 함께 보여 주세요.
      · 고쳐 쓴 문장이 학생이 쓴 문장과 같거나 거의 같으면 그 항목은 빼세요.
    - fix 한 항목에는 어디인지(인용), 왜 고치면 좋은지(까닭), 무엇을 하면 되는지(할 일 한 가지)를 모두 담으세요.
    - praise 도 글에 실제로 있는 것만 칭찬하세요.
    - 고칠 곳을 찾지 못했으면 억지로 만들지 마세요. fix 는 빈 배열이어도 됩니다.
  RULES

  # 책 내용 단정 금지(첨삭·뒷이야기 공용). 모델이 받는 것은 책 제목(뒷이야기는 +지은이)과 학생 글뿐인데
  # 줄거리·인물·결말을 사실처럼 말하던 문제 대응 — 장면을 더 쓰게 할 때는 질문으로 묻게 한다.
  BOOK_FACT_RULES = <<~RULES.freeze
    [책 내용 — 모르는 것은 지어내지 않기]
    - 받는 정보는 책 제목(과 지은이)과 학생 글뿐입니다. 책 내용은 주어지지 않으니 기억이나 짐작에 기대지 마세요.
    - 학생 글에 나오지 않은 줄거리·인물 이름·사건·결말을 사실처럼 쓰지 마세요. 책 제목만 보고 내용을 짐작해 말하지도 마세요.
    - 학생이 쓴 내용이 책과 맞는지 확실히 알 수 없으면, 책과 다르다고 지적하지 마세요.
    - 장면이나 인물 이야기를 더 써 보라고 할 때는 책 내용을 말하지 말고 질문으로 물어보세요(예: '어떤 장면에서 그렇게 느꼈나요?', '그 인물은 왜 그렇게 했을까요?').
  RULES

  # 학생에게 쓰는 말투(첨삭·뒷이야기 공용). 아이를 '당신'이라 부르고, 한 첨삭 안에서 합쇼체('깔끔합니다')와
  # 해요체('노력했어요')를 섞어 "AI 말투"로 읽히던 문제 대응. 지시문 자체는 합쇼체라 그 말투를 흉내 내지
  # 말라는 문장을 함께 둔다(지시문이 모델을 '당신'이라 부르는 것은 괜찮다 — 규칙은 학생에게 쓰는 문장에만).
  CHILD_VOICE_RULES = <<~RULES.freeze
    [말투 — 학생에게 바로 말하기]
    - 학생을 '당신', '학생', '귀하', '너', '친구', '여러분'이라고 부르거나 가리키지 마세요. 부르는 말 없이 바로 말하세요(예: '이 부분이 참 좋아요.', '한 문장 더 써 보세요.').
    - 학생에게 쓰는 문장은 처음부터 끝까지 해요체로만 끝내세요('~해요', '~예요', '~해 보세요', '~할까요?'). '~합니다', '~습니다' 같은 합쇼체나 '~한다', '~했어', '~해 봐' 같은 반말을 한 문장도 섞지 마세요. 이 지시문의 말투를 따라 하지 마세요.
    - 그 학년 아이가 아는 쉬운 낱말로 짧게 쓰세요. '구체성', '통일성', '서사', '성찰', '명료하다' 같은 어려운 말이나 평가 용어는 쓰지 마세요.
    - 번역한 듯한 딱딱한 말은 피하고, 선생님이 옆에서 말하듯 쓰세요(예: '~하는 것이 좋습니다' 대신 '~해 보세요', '~에 대해 서술하세요' 대신 '~을 써 보세요', '매우 인상적입니다' 대신 '~한 부분이 기억에 남아요').
  RULES

  # 나쁜 예 → 좋은 예(첨삭). 실제로 나온 실패(이미 나뉜 두 문장을 합쳐 인용하고 나누라고 함 + 합쇼체 혼용)를
  # 그대로 반례로 보여 준다. 성취기준 코드 금지(학년군 무관 상수).
  REVIEW_EXAMPLE = <<~EXAMPLE.freeze
    [예시 — 형식만 참고하세요. 학생 글에 없는 문장을 인용하면 안 됩니다]
    학생 글: 나는 주인공이 용감하다고 생각했다. 나도 그렇게 해 보고 싶었다.
    - 나쁜 fix: '나는 주인공이 용감하다고 생각했다. 나도 그렇게 해 보고 싶었다'는 문장이 길어요. 나누어 쓰면 더 명확합니다.
      → 이미 마침표로 나뉜 두 문장이라 고칠 곳이 아니에요. '명확합니다'는 합쇼체라 해요체와 섞였어요.
    - 좋은 praise: '나도 그렇게 해 보고 싶었다'라고 마음을 솔직하게 써서 좋아요.
    - 좋은 fix: '나도 그렇게 해 보고 싶었다'에서 무엇을 해 보고 싶었는지 한 가지를 콕 집어 써 보세요. 그러면 생각이 더 또렷하게 전해져요.
    학생 글: 주인공이 불쌍했고 친구도 불쌍했고 나도 슬펐는데 마지막에는 기뻤다.
    - 좋은 fix: '주인공이 불쌍했고 친구도 불쌍했고 나도 슬펐는데 마지막에는 기뻤다'는 한 문장에 생각이 네 개나 이어져 있어서 읽는 사람이 헷갈릴 수 있어요. '친구도 불쌍했다.'에서 한 번 끊어 두 문장으로 나누어 써 보세요.
  EXAMPLE

  # 등급별 지급 포인트. 학년군 무관.
  LEVEL_POINTS = { "A" => 30, "B" => 20, "C" => 10 }.freeze

  # A/B/C 등급 임계값의 단일 진실. `RubricScorable.level_for` 가 판정에 쓰고 `LEVEL_RULE` 이
  # 같은 값을 프롬프트 문구로 렌더한다 — 두 곳에 숫자를 따로 적어 두면 한쪽만 고쳐졌을 때
  # 모델이 안내받은 기준과 앱이 실제 매기는 등급이 어긋난다.
  LEVEL_A_MIN_AVG = 4.0
  LEVEL_A_MIN_LIFE = 4
  LEVEL_B_MIN_AVG = 2.5

  # 프롬프트에 넣는 등급 규칙 문구. 학년군 무관 — 앱은 모든 학년군에 같은 임계값을 적용하므로
  # 학년군마다 다른 규칙을 안내하던 기존 PROMPT_META[:level_rule] 을 이 상수로 일원화했다.
  # 실제 등급은 Report#apply_rubric! 이 5축 가중평균으로 재계산하며 모델이 낸 level 은 쓰지
  # 않는다. 그럼에도 규칙을 알려 주는 이유는 모델이 축 점수를 스스로 검산하게 하기 위해서다.
  LEVEL_RULE = "A는 삶과 연결(life)이 #{LEVEL_A_MIN_LIFE}점 이상이면서 다섯 축 평균이 " \
               "#{LEVEL_A_MIN_AVG} 이상일 때, B는 평균 #{LEVEL_B_MIN_AVG} 이상일 때, 나머지는 C입니다."
  ...
  # 학년군별 제안 개수 상한 접근자. 미지원 band → 기본(:g56).
  def self.feedback_limits(band = DEFAULT_BAND)
    FEEDBACK_LIMITS_BY_BAND.fetch(band, FEEDBACK_LIMITS_BY_BAND.fetch(DEFAULT_BAND))
  end

  # 학년군별 [제안 개수와 순서] 블록(첨삭 프롬프트 주입용). 상한 숫자는 FEEDBACK_LIMITS_BY_BAND 단일 진실.
  def self.build_feedback_limit_rules(band)
    limits = feedback_limits(band)
    <<~RULES
      [제안 개수와 순서 — 한두 가지에 집중]
      - fix 는 최대 #{limits[:fix]}개, grow 는 최대 #{limits[:grow]}개까지만 쓰고, 가장 중요한 것부터 순서대로 쓰세요.
      - fix 와 grow 에 같은 내용을 되풀이하지 마세요. fix 는 이 글에서 고칠 곳, grow 는 다음 글에서 해 볼 한 걸음입니다.
    RULES
  end
  ...
  # **학생 대면 문장 규칙(베타 리뷰 대응)**: 무의미 입력 게이트(REVIEW_NON_ATTEMPT_RULES)를 채점 앞에,
  # 근거·인용(REVIEW_GROUNDING_RULES)·책 내용 단정 금지(BOOK_FACT_RULES)·개수 상한·말투(CHILD_VOICE_RULES)·
  # 나쁜 예/좋은 예(REVIEW_EXAMPLE)를 JSON 스키마 앞에 둔다. JSON 스키마 자체는 뷰·교사 편집이 의존하므로 불변.
  def self.build_rubric_prompt(band)
    ...
    limit_rules = build_feedback_limit_rules(band)
    axis_lines = RUBRIC_AXES.map do |axis|
      "- #{axis}(#{AXIS_LABELS[axis]}, #{codes[axis]}): #{meanings[axis]}"
    end.join("\n")
    anchor_lines = RUBRIC_AXES.map do |axis|
      a = RUBRIC_SCORE_ANCHORS.fetch(axis)
    ...
    <<~PROMPT
      ...
      입력으로는 책 제목과 학생이 쓴 독후감 본문만 주어집니다.

      #{REVIEW_NON_ATTEMPT_RULES}
      ...
      [채점 기준 — 반드시 지키세요]
      - 3점이 이 학년군에서 기대하는 보통 수준입니다. 근거 없이 4점을 주지 마세요.
      - 4·5점은 아래 5점 기준에 해당하는 근거를 본문에서 실제로 찾았을 때만 주세요.
      - 2점은 1점과 3점의 중간, 4점은 3점과 5점의 중간입니다. 해당하는 내용이 아예 없으면 0점입니다.
      - 다섯 축을 각각 따로 채점하세요. 한 축이 좋다고 해서 다른 축을 함께 올리지 마세요.
      - 글이 짧다는 이유만으로 감점하지 말고, 아래 기준에 해당하는 내용이 있는지만 보세요.
      - 등급을 먼저 정한 뒤 축 점수를 거꾸로 맞추지 마세요. 축 점수를 먼저 매기고 등급은 그 결과로 정하세요.

      [축별 점수 기준]
      #{anchor_lines}

      등급 규칙: #{LEVEL_RULE} 포인트는 A=30, B=20, C=10.
