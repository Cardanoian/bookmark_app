...
- `mission.rb` — 학급 단위 독서 미션(**menu_refactor 심화 재설계**: 기간·정량목표·자동배정·정확히-1회 포인트보상). `belongs_to :classroom`·`:created_by`(User, optional, on_delete nullify)·`:book`(optional — 레거시, PR6 드롭 예정), `has_many :mission_goals/:mission_participations`(dependent: :destroy). **status enum**(draft/published/cancelled/archived, 정수 0~3). 검증: title(1..80)·start/end_date 필수·`end_date >= start_date`·`reward_points 0..reward_max_points`(상한=`AppSetting.get("mission_reward_max_points")` 무효 시 `DEFAULT_REWARD_MAX_POINTS=200`, `self.reward_max_points`가 단일 진실로 서버 재검증)·**발행 시 목표≥1**(`published_requires_goal`, Rewarder m6 가드와 짝). 날짜 상태는 DB에 중복 저장 않고 `scheduled?`/`active?`/`ended?`로 `Date.current` 기준 파생. **완료 판정 단일 진실은 `mission_participations.completed_at`**(PR2에서 `ReadingStats#missions` 전환).
