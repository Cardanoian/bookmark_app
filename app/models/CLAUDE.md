...
- `account_merge.rb` — **계정 연동(MERGE) 감사 원장(account_linking_seasons_plan §Phase 2)**. 병합 1건 = 1행. `belongs_to :surviving_user`(생존자=작년 계정, `class_name: "User", optional: true` — DB NOT NULL 이 실제 강제)·`belongs_to :performed_by`(수행자, optional)·`scope :active`(`reversed_at IS NULL`). **`consumed_user_id`(소비된 placeholder 의 원 id)는 belongs_to 없이 보존**한다 — 그 user 행은 병합 중 raw delete 되므로 FK 를 걸 수 없는 역사적 사실이다. `snapshot`(이동행 매니페스트+pre-merge)·`moved_counts`(이동 요약)를 담아 `Accounts::MergeService` 의 근거이자 Phase 4 `reverse!`(14일 시간창 역머지, 미착수)의 소스가 된다. 부분 유니크 `consumed_user_id WHERE reversed_at IS NULL` 로 활성 병합에서 placeholder 1회 소비를 강제한다(동시 이중병합 fail-closed).
