...
- `books/` — 도서 목록·상세 + `_book_card`·`_autocomplete_field`(공용 도서 자동완성 필드 — 표시 input + hidden `book_id` + 결과목록 + 표지, strict/fallback. `book_search` 컨트롤러 부착, 퀴즈·게임·독후감 공용) partial
...
- `reports/` — 독후감 CRUD + `_mode_chooser`·`_photo_capture`·`_form`·`_rubric`·`_body_field`·`_report`·`_report_detail` partial. **새 글 진입은 모드 선택**: `new`가 `params[:input_mode]`(+`report` 프리필 존재)로 3분기 — 파라미터 없음→`_mode_chooser`(직접 쓰기/사진으로 쓰기 카드, `ocr_available?`=false면 사진 카드 비활성), `keyboard`(또는 위저드 프리필)→`_form`, `ocr`→`_photo_capture`(책제목 자동완성 + 카메라/갤러리 단일 file input[`ocr[photo]`] + canvas/preview 압축·미리보기, `POST /ocr`). 사진 업로드는 `OcrController`가 draft 생성 후 `edit` 화면으로 redirect하고 `OcrJob` 방송이 본문 textarea를 채운다. `edit`은 `#ocr_reading_status`(읽는 중/실패 안내)를 두고, OCR 초안 첫 제출(`first_submit`)엔 `_form`이 dirty-check 미부착·"제출하기" 라벨로 렌더한다. **`_form`의 책 제목은 자유텍스트 대신 공용 도서 자동완성**(`books/_autocomplete_field`)으로 받아 선택 시 `book_id`를 연결하고 표지를 자동 표시하되, 미선택 시 기존 `book_title` 자유입력 폴백을 유지한다. `_report_detail`·`_report`는 연결된 `report.book.cover_url` 표지 이미지를 렌더한다. 목록은 중복 화면 제목을 생략하고 학생 메뉴 바로 아래에 `새 독후감 쓰기` 액션을 둔다. 목록 `_report`는 작성 학생에게 확인창이 있는 삭제 버튼을 제공하며, 교사 승인 방송에서도 버튼을 유지한다. **상세(`show`)는 제자리 '수정' 버튼 없이 고쳐쓰기 단일 경로**이고, 상태 의존 영역을 `_report_detail`(local: report, 루트 `dom_id(report, :detail)`)로 분리해 `turbo_stream_from @report` 구독 + AiReviewJob 방송으로 **첨삭/재첨삭 완료를 새로고침 없이 라이브 교체**한다(정책 게이트 버튼은 방송 밖 유지).
