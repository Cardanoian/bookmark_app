...
뷰의 표시 로직을 담는 헬퍼 계층. 주로 도메인 enum/키를 한국어 라벨·Tailwind 색상 클래스로 매핑하거나, 이모지 스프라이트·인라인 SVG 차트처럼 자산/JS 라이브러리 없이 서버 렌더로 표현을 만든다. 라벨/색 상수는 각 헬퍼 모듈 상단에 상수 해시로 둔다.

## 파일
- `application_helper.rb` — 전역 헬퍼(현재 비어 있음, 공통 헬퍼 추가용 자리).
- `monsters_helper.rb` — 몬스터 도감 표시. `monster_emoji`(종→대표 이모지, 시드 이미지 대체)·`element_label`/`element_badge_classes`(속성 라벨·색)·`condition_label`·`condition_progress`(진화 조건 라벨·`ReadingStats` 대비 진행값).
