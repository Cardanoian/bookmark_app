...
#
# 계약:
#   - **멱등**: 이미 summary 가 있거나(book.summary.present?) 이미 Claude 확인을 시도한
#     (book.summary_checked_at.present?) 책은 손대지 않는다(재실행·경쟁·재확인 방지).
#   - **무키 no-op**: ClaudeClient 미설정이면 checked_at 을 세팅하지 않고 즉시 반환한다 —
...
class BookSummaryJob < ApplicationJob
  ...
  def perform(book_id)
    ...
    return unless Ai::ClaudeClient.new.configured? # 무키 → checked_at 미세팅(키 생기면 재시도)
