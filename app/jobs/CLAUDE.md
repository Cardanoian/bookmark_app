...
- `ai_review_job.rb` — 비동기 5축 첨삭. `Ai::ReviewService` 로 루브릭 산출→등급·포인트·유사도(`Ai::VerifyService.max_similarity`)·향상도 저장, 멱등 델타로 포인트 적립 후 교사 검토 큐에 행 방송. **완료·실패 모두 `report` 스트림으로 `reports/_report_detail`(target `dom_id(report,:detail)`)을 replace 방송**해 학생 상세(show)의 5축 결과를 새로고침 없이 라이브 갱신한다(방송 실패는 rescue 로 흡수 — 커밋된 첨삭 결과 보존). 첨삭 자체 실패 시 `:failed`.
