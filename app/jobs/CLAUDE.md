...
- `book_enrichment_job.rb` — **비동기 도서 genre 보강**. 공란 `genre`인 `Book` 을 무API `Books::GenreInference`로 추론해 채운다(네이버 등록 도서용, `SearchService`가 신규 searched 캐시 시 enqueue). 멱등(이미 genre 있으면 no-op)·외부 API 0.
