# 반려 몬스터 성장 서사(docs/monsters.md §5.8)를 앱 표시용으로 로드하는 리더(P2-3).
#
...
#
# 각 라인(dex_no)은 진화 3단계와 1:1로 대응하는 3개 장면(기·승·전·결)과
# 라인 전체를 요약하는 성장 메시지 한 줄로 구성된다. 24개 라인 완전 성장 뒤에는
# 도감 완성 피날레를 별도로 제공한다.
class MonsterLore
  SEED_PATH = Rails.root.join("db/seeds/monster_stories.yml")

  # stage → 장면 제목(진화 단계와 1:1). YAML 은 body 만 담고 제목은 여기서 파생한다.
  SCENE_TITLES = { 1 => "기·승", 2 => "전", 3 => "결" }.freeze

  Scene = Struct.new(:stage, :title, :body, keyword_init: true)
  CompletionFinale = Struct.new(:heading, :paragraphs, keyword_init: true)
  ...
  class << self
    ...
    # 24개 라인의 완전형을 모두 모은 학생에게만 보여 주는 도감 완성 피날레.
    def completion_finale
      @completion_finale ||= begin
        raw = source_data.fetch("completion_finale")
        CompletionFinale.new(
          heading: raw.fetch("heading").to_s.strip,
          paragraphs: raw.fetch("paragraphs").map { |paragraph| paragraph.to_s.strip }.freeze
        )
      end
    end
    ...
    def load_stories
      source_data.fetch("stories").each_with_object({}) do |entry, memo|
    ...
    def source_data
      @source_data ||= YAML.load_file(SEED_PATH)
    end
  end
end
