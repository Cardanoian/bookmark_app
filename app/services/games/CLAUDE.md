...
독서게임 퀴즈 파이프라인(quiz·classic·vocab·whoami)의 **서버 권위 채점**과 **멱등 포인트 적립**을 담는 PORO 계층입니다. 컨트롤러(`app/controllers/games/`)는 얇게 두고, 답안 채점·상한 계산·델타 적립을 이 서비스들에 위임합니다. 정답/정답키/힌트 공개수는 클라이언트로 위임하지 않는 것이 공통 계약입니다(무유출). (book=책 소개 대결은 퀴즈 파이프라인 밖 소셜 도메인이라 이 채점 계층을 쓰지 않습니다.)
