...
- `analytics_controller.rb` — 전교 통합 통계(`show`) + 원자료 CSV 내보내기(`export`, gem 없이 RFC 4180 직접 인코딩). CSV 성공 시 건수 요약을 `AuditLog`에 기록한다. `/admin` 루트.
- `audit_logs_controller.rb` — **P1-3 감사 로그 읽기 화면**. 총괄만 접근하며 최근순 50건 페이지네이션, 작업 종류·실행자 ID·시작/종료일 필터를 제공한다. 감사 원장 자체의 수정·삭제 액션은 없다.
...
- `account_links_controller.rb` — **계정 연동(MERGE) 총괄 보조(account_linking_seasons_plan §Phase 4)**. `index`(전 학교 병합 감사 목록 + 생존자 이름 검색, 페이지네이션 PER_PAGE=25 — **PII 경계: `snapshot` 원본[password_digest·이름]은 뷰에 절대 덤프하지 않고 요약(moved_counts·from/to·수행자·시각·reversed)만 렌더**)·`create`(임의 병합, 총괄 전권 — id 로 old/new 지정, 서비스가 학년도·정지·학생 가드)·`reverse`(member — **시간창 무제한**, `AccountMerge#reverse!` + 커밋 후 `run_reverse_side_effects`[생존자 랭킹/시즌 방송] + **`ReversalError` rescue → alert 리다이렉트**[동시 되돌리기·유니크 충돌 raw 500 방지]). 되돌리기 시간창 제한은 교사(14일)에만 있고 총괄은 없다. **자격증명 purge 후 주의(§Phase 5)**: 야간 `Accounts::PurgeCredentialsJob`(교사 창 14일 경과·미되돌림)이 snapshot 의 `password_digest` 를 nullify 하므로, 그 이후 총괄이 reverse 하면 구조(신원·기록)는 복원되나 **복원된 placeholder 는 비밀번호가 없어 로그인 불가** → 담임이 학생관리에서 비밀번호를 재설정해야 한다.
...
- `users_controller.rb` — 사용자 관리 CRUD + `suspend`/`unsuspend`·`reset_password`·`role`. `reset_password`는 총괄이 **6자 이상 비밀번호를 직접 입력**하고 빈 값·공백·6자 미만이면 기존 다이제스트를 유지하며, 성공 플래시에 평문을 노출하지 않는다. role/suspended 는 전용 액션만(대량할당 차단). **포인트(:points)도 대량할당에서 제외**하고, update 시 목표값과의 차액을 `award_points` 델타(양수)·`spend_points!`(음수)로 조정해 뱃지·진화·랭킹 후크를 태운다(#9, raw 대입 우회 금지). **목표값은 0 이상 정수만 허용**(음수·소수·문자는 저장 없이 정확히 거부)하고 `spend_points!` 실패(잔액 초과) 시 거짓 "수정했어요" 대신 정직히 안내한다. 정보/포인트 변경·정지/해제·비밀번호 초기화·역할 변경 성공은 `AuditLog`에 전후값의 최소 메타데이터와 함께 남긴다. index 페이지네이션(PER_PAGE=50).

학교·도서·퀴즈·뱃지·몬스터·게임 콘텐츠 삭제도 성공한 트랜잭션 안에서 `audit!`를 호출해 P1-3 감사 원장에 남긴다.
