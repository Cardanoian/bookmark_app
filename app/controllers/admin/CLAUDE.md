...
- `account_links_controller.rb` — **계정 연동(MERGE) 총괄 보조(account_linking_seasons_plan §Phase 4)**. `index`(전 학교 병합 감사 목록 + 생존자 이름 검색, 페이지네이션 PER_PAGE=25 — **PII 경계: `snapshot` 원본[password_digest·이름]은 뷰에 절대 덤프하지 않고 요약(moved_counts·from/to·수행자·시각·reversed)만 렌더**)·`create`(임의 병합, 총괄 전권 — id 로 old/new 지정, 서비스가 학년도·정지·학생 가드)·`reverse`(member — **시간창 무제한**, `AccountMerge#reverse!`). 되돌리기 시간창 제한은 교사(14일)에만 있고 총괄은 없다. **자격증명 purge 후 주의(§Phase 5)**: 야간 `Accounts::PurgeCredentialsJob`(교사 창 14일 경과·미되돌림)이 snapshot 의 `password_digest` 를 nullify 하므로, 그 이후 총괄이 reverse 하면 구조(신원·기록)는 복원되나 **복원된 placeholder 는 비밀번호가 없어 로그인 불가** → 담임이 학생관리에서 비밀번호를 재설정해야 한다.
