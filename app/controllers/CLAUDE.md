...
- `dashboard_controller.rb` — 루트(`/`). 역할별 홈 화면으로 분기 렌더(학생·교사·교무·사서). **학생 루트는 홈(menu_refactor 심화 PR5)** — `StudentHomeQuery`로 이어하기·추천/인기 도서·진행 중 미션 요약을 준비한다(전체 기록은 내 서재로 이동). 총괄(superadmin)은 렌더 없이 `/admin` 콘솔로 리다이렉트.
