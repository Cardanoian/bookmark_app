...
- `reading_activities_controller.rb` — **독서활동 허브(menu_refactor 심화 PR5, `resource :reading_activity`)**. book_id 없으면 책 선택(등록 도서 자동완성), 유효한 **등록(비-searched) 도서**면 활동 선택(독후감/게임 5종 진입)+최근 활동+미션 문맥. searched·미존재 book_id 는 무시하고 책 선택 상태로 되돌린다(개인 데이터는 book_id 만으로 조회 안 함). 표현·본인전용이라 `verify_authorized` 스킵 + 비학생 리다이렉트.
