...
- `books_controller.rb` — 도서 카탈로그(`index`)·상세(`show`)·검색(`search`, 네이버 자동완성 JSON, 무키 시 로컬 폴백)·자동완성(`autocomplete`, 로컬 카탈로그[비-searched]만 조회하는 도서 자동완성 JSON `{id,title,author,cover_url,genre,classic}` — genre·classic 은 드롭다운 배지용, 외부호출 0 — 퀴즈·게임 도서 선택용)·**원격검색(`remote_search`**, 검색 버튼 전용 네이버 JSON — `SearchService#remote_search`로 결과 반환+서버가 `book_meta:<isbn>` 캐시 적재, 무키/실패 시 [](로컬 폴백 없음, 타이핑 autocomplete 와 분리); 인가는 `search?` 재사용). index 는 페이지네이션(PER_PAGE=24)하고 **검색 upsert 캐시(`category: searched`)를 카탈로그에서 제외**해 무한 증가를 막는다(#2, searched 는 로컬 검색 폴백에서만 쓰임).
