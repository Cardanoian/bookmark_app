...
class MonstersController < ApplicationController
  ...
  def show
    ...
    # `.submitted` 로 미제출 초안은 뺀다 — OCR 초안도 판독을 마치면 done 이라, 아직 내지도 않은
    # 글을 "곧 반영된다"고 안내하면 거짓말이 된다.
    @awaiting_review_count = current_user.reports.submitted.done.where(reviewed: false).count
    # P2-3 성장 서사(docs/monsters.md §5.8): 보유 학생이 도달한 단계까지 장면을 공개하고,
    ...
    # 24개 라인의 완전형을 모두 모은 뒤에만, 정본의 도감 완성 피날레를 연다.
    completed_lines = current_user.user_monsters.joins(:monster_species)
                                  .where(monster_species: { stage: MonsterSpecies::MAX_STAGE })
                                  .distinct.count(:dex_no)
    @completion_finale = MonsterLore.completion_finale if completed_lines == MonsterSpecies::DESIGN_LINE_COUNT
  end
