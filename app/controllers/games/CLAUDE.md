...
`book`(책 소개 대결)은 **퀴즈 파이프라인 밖 소셜 도메인**으로, Claude/Quiz 를 만들지 않고 도서별 소개 작성·또래 투표(경계=학급)만 합니다.
...
- `attempts_controller.rb` — 퀴즈 파이프라인 표면(quiz·whoami) 결과 기록(`create`). 채점(`QuizPlay`)→`QuizAttempt` 생성/finalize→`award_points` 후 복귀. **whoami 는 선생성 attempt(`attempt_id`)를 finalize**(서버 힌트수 기준 채점). 답안은 타입별(mcq 인덱스/hint 텍스트)이라 `to_i` 로 뭉개지 않고 원형(`to_unsafe_h`)만 넘긴다. 온디맨드(system) 판은 표면 play 로, 교사 퀴즈(id)는 `games_quiz_path` show 로 복귀(`redirect_target`). 실지급 델타 기준 정직한 안내(`result_notice`). **게임 완료 원장 + 미션·챌린지·몬스터 재평가(Phase 3B + menu_refactor 심화 §2.A.3)**: `record_game_play!(game_type: params[:game], ...)`로 원장에 신규 기록되면(중복 재제출이 아니면) `Missions::EvaluateProgress#on_game_play`(미션 진행)·`Challenges::EvaluateProgress#on_game_play`(챌린지 진행)와 `evaluate_monster_unlocks`(몬스터 해금, flash 발견 안내)를 호출한다(모두 신규 GamePlay 일 때만, 미션·챌린지 평가가 몬스터 해금 앞). `book_controller`·`sequel_controller` 도 신규 GamePlay 시 동일.
...
- `book_controller.rb` — **책 소개 대결(소셜 도메인, 퀴즈 파이프라인 밖)**. `play`(book_id)=도서 로드 + 같은 학급 소개 목록(득표순) + 작성 폼 + **정적 작성 가이드(`WRITING_TIPS` 상수, Claude 호출 0)**. `create`=본인·학급으로 소개 작성(`authorize @intro`), 저장 성공 시 `record_game_play!(game_type: :book, ...)`로 원장 기록 후 신규 기록이면 미션 평가(`Missions::EvaluateProgress#on_game_play`)+챌린지 진행 평가(`Challenges::EvaluateProgress#on_game_play`)+몬스터 해금을 재평가한다(book 은 라우트가 game_type 을 서버에서 확정). `vote`/`unvote`=또래 소개 1인 1표(cheer 패턴, RecordNotUnique rescue). 경계 격리는 `BookIntroPolicy`(크로스-학급 차단). **Quiz/GenerateGameContentJob 를 만들지 않는다**(assert).
- `sequel_controller.rb` — **뒷이야기 이어쓰기(창작 소셜 도메인, book_controller 미러)**. `play`(book_id)=도서 로드 + 같은 학급 뒷이야기 목록(득표순) + 작성 폼 + **정적 작성 가이드(`WRITING_TIPS` 상수, 상상 유도, Claude 호출 0)**. `create`=본인·학급으로 저장(`authorize @sequel`), 성공 시 ① `record_game_play!(game_type: :sequel, ...)` ② 신규 기록이면 미션 평가(`Missions::EvaluateProgress#on_game_play`)+챌린지 진행 평가(`Challenges::EvaluateProgress#on_game_play`)+몬스터 해금 재평가 ③ **`SequelFeedbackJob.perform_later(sequel.id)`**(격려형 AI 코멘트 비동기, 무API 폴백). `vote`/`unvote`=또래 뒷이야기 1인 1공감(cheer 패턴, RecordNotUnique rescue). 경계 격리는 `BookSequelPolicy`. AI 코멘트는 **작성자 본인에게만** 노출(Turbo 스트림 라이브 갱신).
