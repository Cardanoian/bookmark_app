...
- `missions_controller.rb` — 담임 학급 독서 미션 관리(**menu_refactor 심화 PR4 재작성**). draft 생성(제목·설명·기간·**고정 2목표**[승인 독후감·게임, `mission[goals][<type>]` target 양수만]·보상 포인트) → **발행(`publish` member)**=`Mission#publish!`로 draft→published 전환 + `AssignmentSync` 학급 학생 자동 배정. 발행 후에는 **제목·설명만 수정**(목표·기간·보상·학급 잠금), 삭제는 draft 만. `index`는 status 필터, `show`는 `ProgressCalculator.batch`로 학생별 진행 현황(N+1 없음). 소유 학급만(`owned_classroom!`, 타 학급 발행 403).
