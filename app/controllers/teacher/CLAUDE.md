...
- `dashboards_controller.rb` — 교사 대시보드(`show`). 독후감 통계·5축 평균·약점 인사이트·향상도·검토 큐 요약(SQL 집계로 경량 로드). 5축 평균은 Relation 을 `axis_averages` 에 넘겨 SQL 경로를 탄다(별도 rubric 행 로드 제거).
