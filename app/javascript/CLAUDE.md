...
- `controllers/report_guide_controller.js` — **직접쓰기 안내 질문 단계(요구 1a)**. 밴드별 질문 답변(name 없는 answer 타깃 textarea)을 `assemble()`가 trim·빈 제외·`\n\n` join 해 숨겨진 `_form`의 `#report_body_field`에 주입(`input`/`change` 이벤트 dispatch)하고 질문 패널을 접어 폼을 노출한다. **`skipLink` 타깃('질문 없이 바로 쓰기' 버튼)도 함께 감춘다** — 그 버튼은 상단 안내 카드(= questions 컨테이너 **밖**)에 있어 질문을 접어도 살아남고, 조립 뒤 남겨 두면 아이가 눌렀을 때 GET 이동이라 방금 만든 본문이 통째로 사라진다(`reset()` 이 되돌린다). **자동 저장은 하지 않는다** — 예전에는 입력할 때마다 localStorage 에 쓰고 '이어서 쓰기' 배너로 복원했으나, 사용자 요구로 그 경로를 걷어내고 **`saveDraft()`(임시 저장)** 로 대체했다: 답변을 조립한 뒤 `_form` 의 `save_draft` 버튼을 `requestSubmit` 해 **서버 초안**으로 저장한다(상태를 브라우저·DB 두 곳으로 가르지 않는다). 그 대가로 '저장을 안 누르고 이탈하면 소실'이라는 실패 모드가 생기므로 `beforeunload` 로 경고한다(저장 제출 직전엔 리스너를 떼어 경고가 뜨지 않게 한다). JS 미로딩 시 질문·폼이 함께 보여 직접 작성 가능(그레이스풀).
